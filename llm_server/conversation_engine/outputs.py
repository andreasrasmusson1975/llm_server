...
High-level reply generation and refinement utilities for conversational LLM applications.

This module provides synchronous and streaming interfaces for generating, reviewing, and refining
assistant replies using Hugging Face Transformers models. It supports multi-stage review pipelines,
streaming output, and code-fenced formatting for improved usability in chatbots and LLM-based assistants.

Key Features:
- Blocking and streaming text generation with configurable sampling parameters.
- Multi-pass review and revision pipelines for answer improvement.
- Utilities for extracting and formatting review sections (improvements, revised answer, comments).
- Type-annotated, well-documented interfaces for integration in conversational AI systems.

Typical usage example:
    >>> from llm_improvement.conversation.outputs import get_and_improve_reply
